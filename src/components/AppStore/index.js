...
class AppStore extends Component {
  state = {searchInput: '', tabsItemId: tabsList[0].tabId}
  changeTab = id => {
    this.setState({tabsItemId: id})
  }
  onClickValue = event => {
    this.setState({searchInput: event.target.value.toLowerCase()})
  }
  render() {
    const {searchInput, tabsItemId} = this.state
    const searchResults = appsList.filter(each => each.category === tabsItemId)
    const finalSearchResults = searchResults.filter(each =>
    ...
    return (
      <div className="container">
        ...
        <input
          type="search"
          placeholder="Search"
          onChange={this.onClickValue}
        />
        <img alt="search icon" src="https://assets.ccbp.in/frontend/react-js/app-store/app-store-search-img.png"/>
        <ul className="tabs-list">
          {tabsList.map(each => (
            <TabItem each={each} key={each.tabId} changeTab={this.changeTab} />
          ))}
        </ul>
        <ul className="app-list">
          {finalSearchResults.map(each => (
